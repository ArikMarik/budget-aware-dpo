...
def main():
    ...
    logger.info("Easy (GSM8K) acc:  %.2f%% (%d/%d)", metrics["accuracy_easy"] * 100, metrics["num_easy_correct"], metrics["num_easy"])
    logger.info("Hard (MATH) acc:   %.2f%% (%d/%d)", metrics["accuracy_hard"] * 100, metrics["num_hard_correct"], metrics["num_hard"])
